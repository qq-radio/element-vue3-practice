<template>
  ...
  >
    <template #default="{ row, $index, ...rest }">
      <!-- 显示出来的按钮 -->
      <template
        v-for="buttonRow in getSubButtons(row, $index).preButtons"
        :key="buttonRow.text"
      >
      ...
      >
        <span class="plus-table-action-bar__dropdown__link">
          <span class="plus-table-action-bar__more-text">
            {{ t("plus.table.more") }}</span
          >
...
const formRefs = inject(TableFormRefInjectionKey) as Ref<
...
const render = (
  ...
  if (props.type === "icon") {
    return h(
      ...
      {
        placement: "top",
        content: unref(buttonRow.text) as string,
        ...buttonRow.tooltipProps,
      },
      () =>
        withDirectives(
          h(
            ElIcon,
            {
              ...
              onClick: (event: MouseEvent) =>
                handleClickAction(row, buttonRow, index, event, rest),
            },
...
const handleClickAction = (
  ...
  if (buttonRow.confirm) {
    ...
    if (
      isPlainObject(buttonRow.confirm) &&
      typeof buttonRow.confirm !== "boolean"
    ) {
      const tempTitle = isFunction(buttonRow.confirm.title)
        ? (buttonRow.confirm.title as (data: ButtonsCallBackParams) => string)(
            callbackParams
          )
      ...
      const tempMessage = isFunction(buttonRow.confirm.message)
        ? (
            buttonRow.confirm.message as (data: ButtonsCallBackParams) => string
          )(callbackParams)
